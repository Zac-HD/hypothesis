...
def ir_to_bytes(ir: Iterable[IRType], /) -> bytes:
    ...
    for elem in ir:
        ...
        if isinstance(elem, float):
            ...
            tag = 2 << 5
            nbits = elem.bit_length() + (8 if elem < 0 else 7)
            elem = elem.to_bytes(nbits // 8, "big", signed=True)
        elif isinstance(elem, bytes):
            tag = 3 << 5
        else:
            assert isinstance(elem, str)
            tag = 4 << 5
            elem = elem.encode(errors="surrogateescape")
...
def ir_from_bytes(buffer: bytes, /) -> List[IRType]:
    ...
    while idx < len(buffer):
        ...
        if tag == 1:
            ...
            assert tag == 4
            parts.append(chunk.decode(errors="surrogateescape"))
    return parts
