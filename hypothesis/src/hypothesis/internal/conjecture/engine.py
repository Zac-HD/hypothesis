...
class ConjectureRunner:
    ...
    def observe_for_provider(self):
        if (
            ...
            # Wrap provider's callbacks so they only fire for observations
            # generated by this provider (not after switching to hypothesis)
            def wrap(
                cb: Callable[[Observation], None],
            ) -> Callable[[Observation], None]:
                def wrapped(observation: Observation) -> None:
            ...
            provider_config = dataclasses.replace(
                self.provider.observability,
                callbacks=tuple(
                    wrap(cb) for cb in self.provider.observability.callbacks
                ),
            )
    ...
    def _maybe_evict_observations(self) -> None:
        ...
        if not _have_evicted_observations:
            ...
            for f in storage_directory("observed", intent_to_write=False).path.glob(
                "*.jsonl"
            ):  # pragma: no cover
                if f.stem < max_age:
